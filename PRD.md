...
10. Polish: My Chart list, App Store assets, EAS build + TestFlight
    ...
    - ✅ **Monetization / Natal Plus (Build Phase 10 paywall).** Authority: `MONETIZATION.md`. RevenueCat (`react-native-purchases`) configured via `EXPO_PUBLIC_REVENUECAT_APPLE_API_KEY` in `.env` → `app.config.ts` `extra`; `lib/subscription.ts` exposes `useIsPlus()` (entitlement `plus`); shared `components/PaywallSheet.tsx` with runtime price strings, Restore, and a clear dismiss ✕. Placements: Echo tease, one-time post–Big 3 soft paywall, contextual gates (Connection #2+, Learn 3+, transit calendar, pattern insights, chart interpretations). Analytics events fire with `source` (sink is `lib/analytics.ts` — console in `__DEV__` until a real provider is wired). **End-to-end verified** on a physical device: real RevenueCat sandbox purchase (annual, with trial) completed successfully and `useIsPlus()` unlocked gated features app-wide without a restart. Two code bugs found and fixed along the way: `syncPurchasesUser()` called `Purchases.logOut()` unconditionally on cold start before sign-in, which RevenueCat throws on when the current user is already anonymous (the default pre-sign-in state) — guarded with `Purchases.isAnonymous()`; and `PaywallSheet`'s Monthly row hardcoded `"No trial"` while `trialDetail()` hardcoded a `/yr` suffix, so Monthly's real 7-day trial never rendered — `trialDetail()` is now period-aware for both rows. Also required two dashboard-only fixes (no code): the RevenueCat "current" offering was pointed at an empty offering with no packages attached, and both App Store Connect subscriptions needed metadata (localization + price) completed before StoreKit would resolve them.
